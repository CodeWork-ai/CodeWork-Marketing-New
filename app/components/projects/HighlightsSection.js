...
export default function HighlightSection() {
  ...
  return (
    <section className="relative min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white flex flex-col lg:flex-row items-center justify-between px-6 lg:px-20 py-16 overflow-hidden">
      {/* Text Content */}
      <motion.div
        className="lg:w-1/2 z-10 space-y-6 lg:space-y-8 text-center lg:text-left"
      ...
      >
        ...
        <p className="text-base lg:text-lg text-[#dceaee]">
          {activeProject.description}
        </p>
        <ul className="space-y-3 lg:space-y-4 text-white">
          {activeProject.points.map((point, index) => (
            <motion.li
              key={index}
              className="flex items-start text-sm lg:text-base group"
      ...
      {/* Image Section */}
      <motion.div
        className="lg:w-1/2 relative flex justify-center items-center z-10 mt-12 lg:mt-0"
      ...
      >
        ...
        <motion.div
          ...
          style={{
            boxShadow:
              "0 0 40px 10px rgba(253, 98, 98, 0.5), 0 0 80px 20px rgba(173, 216, 230, 0.3)",
          }}
        >
          <img
        ...
        >
          {projects.map((project, index) => (
            <div
              key={project.id}
              style={{
                transform: `rotate(${index * 120}deg) translate(12rem) rotate(-${index * 120}deg)`,
              }}
              className="absolute"
            >
              <motion.div
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-20 left-10 w-48 h-48 bg-gradient-to-r from-pink-500 to-red-500 opacity-20 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute bottom-20 right-10 w-64 h-64 bg-gradient-to-r from-blue-500 to-teal-500 opacity-20 rounded-full blur-3xl animate-pulse"></div>
      </div>
    </section>
  );
}
